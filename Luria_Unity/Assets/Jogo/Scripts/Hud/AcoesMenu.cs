	...
	public void listarObjetivos(Objetivo[] objetivos)
	{
		...
		for (int i = 0; i < areaObjetivos.transform.childCount; i++)
		{
			...
			if (i < objetivos.Length)
			{
				...
				objPrefab.Find("Titulo").GetComponent<Text>().text = titulo;

				if (objetivos[i].estado == 0)
				{
					if (objetivos[i].obrigatorio)
						objPrefab.Find("Icone").GetComponent<Image>().sprite = objAndamento;
					else
						objPrefab.Find("Icone").GetComponent<Image>().sprite = objSecundario;
				}
				else if (objetivos[i].estado == 1)
					objPrefab.Find("Icone").GetComponent<Image>().sprite = objConcluido;
				else
					objPrefab.Find("Icone").GetComponent<Image>().sprite = objFalhado;
			}
			else
